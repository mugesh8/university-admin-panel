  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`relative flex h-full min-h-0 flex-col px-4 py-6 lg:py-8 ${collapsed ? 'lg:px-2.5' : 'lg:px-5'}`}
        >
          <div
            className={`mb-4 flex shrink-0 items-center gap-3 border-b border-white/10 pb-4 ${collapsed ? 'lg:flex-col lg:items-center lg:gap-2 lg:border-0 lg:pb-2' : ''}`}
          >
            <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-white/10 bg-white/[0.06] p-1 ring-1 ring-white/10">
              <img src={MUCM_CREST_URL} alt="" className="h-9 w-9 object-contain" />
            </div>
            {!collapsed ? (
          ...
          <nav
            className={`min-h-0 flex-1 space-y-2.5 overflow-y-auto overflow-x-hidden overscroll-y-contain pr-1 [scrollbar-color:rgba(255,255,255,0.25)_transparent] [scrollbar-width:thin] ${
            ...
            {sidebarNav.map((item, index) => {
              if (item.type === 'section') {
                return collapsed ? null : (
                  <div key={`section-${item.label}-${index}`} className="pt-3 first:pt-0">
              ...
              <NavLink
                ...
                onClick={onCloseMobile}
                className={({ isActive }) =>
                  `relative flex w-full items-center gap-3 overflow-hidden rounded-2xl border text-left transition-all duration-300 ${
                    isActive
                  ...
                  } ${collapsed ? 'lg:justify-center lg:px-0 lg:py-2.5' : 'px-3.5 py-3'}`
                }
              >
                {({ isActive }) => (
                  <>
                    ...
                    <span
                      className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border ${
                        ...
                        collapsed
                          ? 'lg:h-8 lg:w-8 lg:rounded-md lg:border-0 lg:bg-transparent lg:text-current'
                          : ''
                      }`}
                    >
                      <NavIcon className="h-[18px] w-[18px] shrink-0" strokeWidth={1.65} aria-hidden />
                    </span>
                    {!collapsed ? (
          ...
          <div className="mt-auto hidden shrink-0 border-t border-white/10 pt-3 lg:block">
            <button
              type="button"
              onClick={onToggleCollapse}
              aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
              className={`flex w-full items-center gap-2 rounded-xl py-2.5 text-sm font-medium text-white/55 transition hover:bg-white/[0.06] hover:text-white/90 ${
